...
namespace UrbamidAddIn_V10_2.Utility
{
    public class Utility : Log
    {
        ...
        internal bool getLabelStaus(string layerDaImportare)
        {
        ...
        internal string getFileStyleName(string nomeLayer)
        {
            IMap map;
            IEnumLayer layerTag;
            ILayer layer;
            ...
            try
            {
                ...
                layerTag = map.Layers;
                layer = layerTag.Next();
                bool layerTrovato = false;

                for (int index = 0; index < map.LayerCount; index++)
                {
                    if (map.Layer[index].Name == nomeLayer)
                    {
                        selectedLayer = map.Layer[index];
                        layerTrovato = true;
                        break;
        ...
        internal List<String> getListaLayer()
        {
            ...
            try
            {
                layers.Add("Seleziona\\Deseleziona tutti");
                map = ArcMap.Document.FocusMap;
                layerTag = map.Layers;
                layer = layerTag.Next();
                while (layer != null)
                ...
                try
                {
                    ...
                    while (selectedLayer != null)
                    {
                        ...
                        catch (Exception exx)
                        {
                            ...
                            scriviLog(exx.Message + " --> " + exx.StackTrace);
                        }
                        finally
                        {
                            selectedLayer = layerTag.Next();
