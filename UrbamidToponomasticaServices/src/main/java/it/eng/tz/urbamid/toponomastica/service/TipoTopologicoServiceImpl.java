...
import it.eng.tz.urbamid.toponomastica.persistence.repositories.JpaRepositoryTipoTopologico;
import it.eng.tz.urbamid.toponomastica.web.dto.TipoTopologicoDTO;
import it.eng.tz.urbamid.toponomastica.web.dto.converter.TipoTopologicoConverter;

@Service
public class TipoTopologicoServiceImpl implements TipoTopologicoService {

	private static final Logger logger = LoggerFactory.getLogger(TipoTopologicoServiceImpl.class.getName());
	...
	@Autowired
	private JpaRepositoryTipoTopologico repository;
	
	@Autowired
	private TipoTopologicoConverter converter;
	
	@Override
	public List<TipoTopologicoDTO> findAll() throws ToponomasticaServiceException {
		
		String idLog = "findAll TipoTopologico";
		
		try {
			
			logger.info(START, idLog);
			
			List<TipoTopologico> result = repository.findAll(new Sort(Sort.Direction.ASC, "descrizione"));
