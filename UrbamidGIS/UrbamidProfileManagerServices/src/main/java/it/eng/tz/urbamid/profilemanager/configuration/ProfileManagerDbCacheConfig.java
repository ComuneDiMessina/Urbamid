...
@Configuration
@EnableCaching
public class ProfileManagerDbCacheConfig extends CachingConfigurerSupport {
	@Autowired
	...
	private CacheManager createCacheManager() {
		long dimensioneCache = Long.parseLong(env.getProperty("profilemanager.db.cache.cache.size"));
		long ttlMillisecondi = Long.parseLong(env.getProperty("profilemanager.db.cache.cache.ttl"));
	...
	private Map<String, org.ehcache.config.CacheConfiguration<?, ?>> createCacheConfigurations(
		...
		for (BeanDefinition bd : scanner
				.findCandidateComponents("it.eng.tz.urbamid.profilemanager.persistence.model")) {
