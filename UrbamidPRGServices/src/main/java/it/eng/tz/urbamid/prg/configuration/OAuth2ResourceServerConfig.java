...
public class OAuth2ResourceServerConfig extends ResourceServerConfigurerAdapter {
	...
	public void configure(final ResourceServerSecurityConfigurer config) {
		...
		.resourceId(env.getProperty("prg.oauth.resource.id"));
	}
	...
	{
		return new JwkTokenStore(env.getProperty("prg.oauth.jwt.key.url"), accessTokenConverter());
	}
	...
	public JwtClaimsSetVerifier issuerClaimVerifier() {
		try {
			return new IssuerClaimVerifier(new URL(env.getProperty("prg.oauth.jks.claim.issuer.url")));
...
//        ImplicitResourceDetails resource = new ImplicitResourceDetails();
	...
	CorsConfigurationSource corsConfigurationSource() {
		...
		boolean abilitaCors = Boolean.parseBoolean(env.getProperty("prg.web.cors.enbaled"));
		if( abilitaCors )
		{
