...
@Configuration
@EnableCaching
public class PRGDbCacheConfig extends CachingConfigurerSupport {
	@Autowired
	...
	private CacheManager createCacheManager() {
		long dimensioneCache = Long.parseLong(env.getProperty("prg.db.cache.cache.size"));
		long ttlMillisecondi = Long.parseLong(env.getProperty("prg.db.cache.cache.ttl"));
	...
	private Map<String, org.ehcache.config.CacheConfiguration<?, ?>> createCacheConfigurations(
		...
		for (BeanDefinition bd : scanner
				.findCandidateComponents("it.eng.tz.urbamid.prg.persistence.model")) {
